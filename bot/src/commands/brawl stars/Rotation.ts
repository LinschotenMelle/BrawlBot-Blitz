...
function createEmbed(
  ...
  maps.forEach((map) => {
    ...
    const value = isOngoing
      ? `Until ${formattedDate}`
      : `Arrives on ${formattedDate}`;
...
export default new Command({
  ...
  run: async ({ interaction }) => {
    ...
    const upComingMaps = brawlStarsMaps.filter(
      (m) => convertTimestamp(m.startTime).getTime() > currenttime.getTime()
    );
    ...
    if (upComingMaps.length > 1) {
      embeds.push(
        createEmbed("Upcoming Rotation of Maps", upComingMaps, false)
      );
    }
